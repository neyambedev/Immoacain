document.addEventListener("DOMContentLoaded", () => {
    ...
    // Détecter la catégorie selon l'URL courante
    function getCategorieFromPath() {
        const path = window.location.pathname;
        if (path.includes('/locations/appartements')) return 'appartement';
        if (path.includes('/locations/chambres'))     return 'chambre';
        if (path.includes('/locations') && !path.includes('/hotels')) return 'appartement,chambre';
        return '';
    }
    ...
    const rechercher = () => {
        ...
        timer = setTimeout(() => {
            ...
            const categorie = getCategorieFromPath();
            const params = new URLSearchParams({ q });
            if (categorie) params.set('categorie', categorie);

            fetch(`/filtrer_terrains?${params.toString()}`)
                .then(r => r.json())
                .then(data => {
